...
$( document ).ready(function() {
  ...
  $('header.header-auto-height, header#main').css('height',$(window).width()/1.7 +'px');

	function JSmediaQueries () {
		if (document.body.clientWidth < 620) {
			var small = true;
		} else if (document.body.clientWidth > 621) {
			small = false;
		};
		if (small === true) {
      $('.mobile-full-height').css({minHeight: $(window).height() + 'px'});
      $('.full-height').css({minHeight: $(window).height() + 85 + 'px'});
      $('ul.main-nav').each(function (i){
	...
	$('.animate > *').transition({opacity: 0.04, y: "50px"}, 0);
	$('.animate').each(function (i){
		...
		}, {offset: function(){return $(window).height() / 1.3}});
	});
  $('.animate-left > *').transition({opacity: 0.02, x: "-50px"}, 0);
  ...
  $('.animate-right > *').transition({opacity: 0.02, x: "50px"}, 0);
  ...
  $('.animate-down > *').transition({opacity: 0.02, y: "-50px"}, 0);
  $('.animate-down').each(function (i){
    var waypoints = $(this).waypoint(function(direction) {
      $("> *", this).each(function (i){
          var a = (i + i * 150);
          $(this).delay(a).transition({opacity: 1, y: "0"}, 700, function(){
  ...
  $('.animate-blur > *').transition({opacity: 0, y: "50px", filter: "blur(10px)"}, 0);
  $('.animate-blur').each(function (i){
    var waypoints = $(this).waypoint(function(direction) {
      $("> *", this).each(function (i){
          var a = (i + i * 150);
  ...
  $('.animate-count').each(function (i){
	...
	$('.main-nav .btn-toggle').click(function(event){
		...
		if(NavisOpen) {
			$(x).css({display: "block", zIndex: "90", height: $(window).height() + 100 + 'px'});
        ...
        $('ul.main-nav').each(function (i){
          $("> li", this).each(function (i){
			...
			$(x).transition({opacity: 0, y:'-30px'}, 200, 'snap', function(){
				$(x).css({display: "none", zIndex: "-10000", height: $(window).height() + 100 + 'px'});
  ...
  $('.btn-submenu-dropdown').each(function (i){
  ...
  $('[data-function=scrollto]').each(function (i){
...
/*	if (open != false) {
		$('html').on('click',function(){
			$('html').find('nav, div, ul, button').removeClass('drawer-open').removeClass('open');
		});
	};
*/
	...
	var lastScrollTop = 0;
	var delta = 5;
	var navbarHeight = $('nav.main-nav').outerHeight();
	...
	function hasScrolled() {
		if (document.body.clientWidth < 680) {
	        ...
	        // Scroll Down
	        $('nav.main-nav').removeClass('nav-down').addClass('nav-up');
	        //$('nav.main-nav #main-nav').removeClass('drawer-open');
	        //$('nav.main-nav .btn-toggle.main-menu').removeClass('drawer-open');
          //$('.btn-submenu-dropdown').removeClass('drawer-open');
	        /*$('nav.main-nav #main-nav').transition({opacity: 0, y:'30px'}, 400, 'snap', function(){
    				$('nav.main-nav #main-nav').css({display: "none", zIndex: "-10000"});
    				NavisOpen = false;
    			});*/
		    } else {
		        // Scroll Up
